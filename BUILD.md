...
| AND-005 | android | Implementer l'ecran `Search` avec orchestration local + online | in_progress | AND-002, AND-003, SRV-002 | `docs/android/screens/search.md`, `docs/product/user-flows.md`, `docs/server/api-contract.md` | TabRow Bibliothèque/En ligne posé, LocalLibrarySearchTab avec like+playlist, OnlineSearchTab avec playlist. Affichage online limité aux titres pour l'instant (artistes/albums online TODO) |
...
- Conserver la fusion local + online cote Android : priorite d'affichage au local pour `Meilleur resultat` si le match local est fort, online en sections dediees.
...
- Nettoyer `SearchScreen` des boutons de test et remplacer le placeholder actuel par la vraie structure : barre sticky, suggestions locales, hero `Meilleur resultat`, bloc local, sections online.
