...
/**
 * Ecran de navigation dans la bibliotheque locale pour Android Auto.
 * Affiche la liste des titres telecharges hors-ligne.
 */
class LibraryBrowseScreen(
    ...
    init {
        // Annulation propre de la coroutine lors du detachement de l'ecran pour eviter les fuites de CarContext
    ...
    private fun loadData() {
        screenScope.launch {
            try {
                tracksList = repository.getDownloadedTracks()
            } catch (e: Exception) {
                android.util.Log.e("LibraryBrowseScreen", "Erreur lors du chargement des morceaux", e)
            } finally {
                isLoaded = true
                invalidate()
            }
    ...
    override fun onGetTemplate(): Template {
        if (!isLoaded) {
            return ListTemplate.Builder()
                .setTitle("Bibliothèque AURA")
                .setLoading(true)
                .build()
        }

        val listBuilder = ItemList.Builder()
            .setNoItemsMessage("Aucun titre hors-ligne disponible.")
            ...
            if (file.exists()) {
                try {
                    val options = android.graphics.BitmapFactory.Options().apply {
                        inSampleSize = 4 // Downsample to avoid memory overhead
                    }
                    ...
                    android.util.Log.e("LibraryBrowseScreen", "Error decoding cover bitmap", e)
                }
            }
        }
    ...
    private fun playTrack(trackId: String) {
        ...
        // Envoi de la commande de lecture directement a l'orchestrateur partage de l'app
